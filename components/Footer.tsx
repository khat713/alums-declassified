...
export default function Footer() {
  return (
    <footer style={{ background: 'linear-gradient(135deg, #0a0f1e 0%, #0f172a 100%)', borderTop: '1px solid rgba(13,124,126,0.15)', marginTop: '5rem', position: 'relative', zIndex: 1 }}>
      <div style={{ height: '3px', background: 'linear-gradient(90deg, #0d7c7e, #fbbf24, #0d7c7e)', opacity: 0.6 }} aria-hidden="true" />
      <div className="container mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <nav aria-label="Course navigation">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
              {[['About', '/about'], ['Start Here', '/start-here'], ['Modules', '/modules'], ['Assignments', '/assignments'], ['Tech Help', '/tech-help']].map(([label, href]) => (
                <Link key={href} href={href} style={{ color: 'rgba(255,255,255,0.75)', fontSize: '0.875rem', textDecoration: 'none', transition: 'color 0.15s' }}
          ...
          <nav aria-label="Modules navigation">
            <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '1rem' }}>Modules</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
